...
namespace laba1_s2_
{
    class Class3
    {
        ...
        {
            wn.Title = "Калькулятор"; wn.Height = 500; wn.Width = 300; wn.Background = Brushes.LightCoral;
            Grid grid = new Grid();
            ...
            for (int i = 4; i >= 2; i--)
                for (int j = 0; j < 3; j++)
                {
                    ...
                    butt.Content = k; butt.FontSize = 24; butt.FontFamily = new FontFamily("Comic Sans MS");
            ...
            Button AC = new Button(); AC.Content = "AC"; AC.FontSize = 24; AC.FontFamily = new FontFamily("Comic Sans MS");
            Grid.SetRow(AC, 1); Grid.SetColumn(AC, 0); AC.Click += AC_Click;
            Button Sign = new Button(); Sign.Content = "+/-"; Sign.FontSize = 24; Sign.FontFamily = new FontFamily("Comic Sans MS");
            Grid.SetRow(Sign, 1); Grid.SetColumn(Sign, 1); Sign.Click += Button_Sign;
            Button Modulo = new Button(); Modulo.Content = "%"; Modulo.FontSize = 24; Modulo.FontFamily = new FontFamily("Comic Sans MS"); Modulo.Tag = "Modulo";
            Grid.SetRow(Modulo, 1); Grid.SetColumn(Modulo, 2); Modulo.Click += Button_Op;
            Button Plus = new Button(); Plus.Content = "+"; Plus.FontSize = 24; Plus.FontFamily = new FontFamily("Comic Sans MS"); Plus.Tag = "Add";
            Grid.SetRow(Plus, 5); Grid.SetColumn(Plus, 2); Grid.SetColumnSpan(Plus, 2);
            Button Minus = new Button(); Minus.Content = "-"; Minus.FontSize = 24; Minus.FontFamily = new FontFamily("Comic Sans MS"); Minus.Tag = "Substract";
            Grid.SetRow(Minus, 4); Grid.SetColumn(Minus, 3); Minus.Click += Button_Op;
            Button Divide = new Button(); Divide.Content = ":"; Divide.FontSize = 24; Divide.FontFamily = new FontFamily("Comic Sans MS"); Divide.Tag = "Divide";
            Grid.SetRow(Divide, 3); Grid.SetColumn(Divide, 3); Divide.Click += Button_Op;
            Button Mult = new Button(); Mult.Content = "*"; Mult.FontSize = 24; Mult.FontFamily = new FontFamily("Comic Sans MS"); Mult.Tag = "Multiply";
            Grid.SetRow(Mult, 2); Grid.SetColumn(Mult, 3); Mult.Click += Button_Op;
            Button Zero = new Button(); Zero.Content = 0; Zero.FontSize = 24; Zero.FontFamily = new FontFamily("Comic Sans MS");
            Grid.SetRow(Zero, 5); Grid.SetColumn(Zero, 1); Zero.Click += Button_Num;
            Button Result = new Button(); Result.Content = "="; Result.FontSize = 24; Result.FontFamily = new FontFamily("Comic Sans MS");
            Grid.SetRow(Result, 6); Grid.SetColumn(Result, 0); Grid.SetColumnSpan(Result, 2); Result.Click += Result_Click;
            Button Float = new Button(); Float.Content = ","; Float.FontSize = 24; Float.FontFamily = new FontFamily("Comic Sans MS");
            Grid.SetRow(Float, 5); Grid.SetColumn(Float, 0); Float.Click += Button_Float;
            Button Sqrt = new Button(); Sqrt.Content = "sqrt"; Sqrt.FontSize = 24; Sqrt.FontFamily = new FontFamily("Comic Sans MS");
            Grid.SetRow(Sqrt, 1); Grid.SetColumn(Sqrt, 3); Sqrt.Click += Button_Sqrt;
            Button tomain = new Button(); tomain.Content = "Назад в меню"; tomain.FontSize = 20; tomain.FontFamily = new FontFamily("Comic Sans MS");
            Grid.SetRow(tomain, 6); Grid.SetColumn(tomain, 2); Grid.SetColumnSpan(tomain, 2); tomain.Click += _4ToMain_Click;

            ResultBox.FontSize = 48; ResultBox.FontFamily = new FontFamily("Comic Sans MS"); ResultBox.TextAlignment = TextAlignment.Right; ResultBox.Text = "0";
